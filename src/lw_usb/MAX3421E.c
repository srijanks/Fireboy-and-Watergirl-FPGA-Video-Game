...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE sendbuffer[1 + nbytes];
		...
		//read return code from SPI peripheral
		Status = XSpi_Transfer(&SpiInstance, sendbuffer, NULL, (nbytes + 1));
		//if return code != 0 print an error
		if (Status != XST_SUCCESS) {
			xil_printf ("SPI device failed to write %d", Status);
		}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE receivebuffer[nbytes + 1];
		...
		//write reg via SPI
		receivebuffer[0] = reg;
		Status = XSpi_Transfer(&SpiInstance, receivebuffer, receivebuffer, (nbytes+1));
		//if return code != 0 print an error
		if (Status != XST_SUCCESS) {
			xil_printf ("SPI device failed to read %d", Status);
		}
